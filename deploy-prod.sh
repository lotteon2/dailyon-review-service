...
if kubectl rollout status deployment/review-deployment -n prod | grep -q "successfully rolled out"; then
  echo "Success to create new kubernetes deployment resources"

  kubectl annotate deployment/review-deployment kubernetes.io/change-cause="Change image to ${IMAGE_TAG}"
  ...
  kubectl rollout undo deployment/review-deployment -n prod
  if kubectl rollout status deployment/review-deployment -n prod | grep -q "successfully rolled out"; then
    echo "Rollback was successful"

    kubectl annotate deployment/review-deployment kubernetes.io/change-cause="Rollback to previous image"
  else
